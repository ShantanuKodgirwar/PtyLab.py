...
class OPR(BaseEngine):
    ...
    def reconstruct(self):
        ...
        for i, mode in enumerate(self.OPR_modes):
            ...
            self.reconstruction.probe_stack[0, 0, i, 0, :, :, :] = cp.repeat(
                self.reconstruction.probe[0, 0, mode, 0, :, :, cp.newaxis], Nframes, axis=2)
    ...
    def orthogonalizeProbeStack(self, probe_stack, n_dim):
        ...
        for i, mode in enumerate(self.OPR_modes):
            ...
            if self.params.OPR_tsvd_type == 'randomized':
                ...
                U, s, Vh = cp.linalg.svd(probe_stack[:, :, i, :, :, :].reshape(
                    n * n, nFrames), full_matrices=False)
                s[n_dim:] = 0
            ...
            if self.params.OPR_neighbor_constraint:
                ...
                probe_stack[:, :, i, :, :, :] = self.alpha * probe_stack[:, :, i, :,
    ...
    def probeUpdate(self, objectPatch: np.ndarray, DELTA: np.ndarray, weight: float, gimmel=0.1):
        ...
        r = self.reconstruction.probe + self.betaProbe * \
            xp.sum(frac * DELTA, axis=(0, 1, 3), keepdims=True)
        return r
